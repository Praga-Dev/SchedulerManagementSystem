...
namespace SchedulerManagementSystem.DataAccess.Repositories
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        private static List<EmployeeInfoDB> _EmployeeInfoList = new()
            {
                new() {Id = new("4A053397-E1FC-43F8-815C-93ACC81EB94F"), FirstName = "Alex", MiddleName="F", LastName= "Farrell", GradeId = new("2F880FC2-87A4-41FA-A1C9-7481B4ED0E68"), LocationId = new("455765AA-03A5-40C0-9997-242470DFA9AE"),TotalHours = 0 },
        ...
        public async Task<Response<Guid>> CreateEmployee(EmployeeInfoDB employeeInfoDB, Guid loggedInUserId)
        {
            ...
            try
            {
                // TODO Check for mandatory fields in StorProc
        ...
        public async Task<Response<Guid>> UpdateEmployee(EmployeeInfoDB employeeInfoDB, Guid loggedInUserId)
        {
            ...
            try
            {
                if (employeeInfoDB != null && Helpers.IsValidGuid(employeeInfoDB.Id))
                {
                    ...
                    if (employee != null)
                    {
                        employee = employeeInfoDB;

                        //TODO Check
                        //employee.FirstName = employeeInfoDB.FirstName;
                        //employee.MiddleName = employeeInfoDB.MiddleName;
                        //employee.LastName = employeeInfoDB.LastName;
                        ...
                        //// CHECK ON DB todo check the value is valid or not
                        //employee.DepartmentId = employeeInfoDB.DepartmentId;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.BudgetDepartmentId = employeeInfoDB.BudgetDepartmentId;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.TitleId = employeeInfoDB.TitleId;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.EmployeeRoleId = employeeInfoDB.EmployeeRoleId;
                        
                        //// CHECK ON DB todo check the value is valid or not
                        //employee.LocationId = employeeInfoDB.LocationId;

                        //employee.WorkHoursPerWeek = employeeInfoDB.WorkHoursPerWeek;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.DirectReportId = employeeInfoDB.DirectReportId;

                        //employee.LoginID = employeeInfoDB.LoginID;
                        //employee.IAD_StartDate = employeeInfoDB.IAD_StartDate;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.HireTypeId = employeeInfoDB.HireTypeId;
                        //employee.TimeSheetExempt = employeeInfoDB.TimeSheetExempt;
                        //employee.Allocation = employeeInfoDB.Allocation;


                        //// CHECK ON DB todo check the value is valid or not
                        //employee.ATS_Admin_LevelID = employeeInfoDB.ATS_Admin_LevelID;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.ATS_Security_LevelID = employeeInfoDB.ATS_Security_LevelID;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.BPA_Management_LevelID = employeeInfoDB.BPA_Management_LevelID;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.RITA_LevelID = employeeInfoDB.RITA_LevelID;

                        //// CHECK ON DB todo check the value is valid or not
                        //employee.BPA_LevelID = employeeInfoDB.BPA_LevelID;

                        //employee.AIG_EmployeeNumber = employeeInfoDB.AIG_EmployeeNumber;
                        //employee.PhoneNumber = employeeInfoDB.PhoneNumber;
                        //employee.Email = employeeInfoDB.Email;
                        //employee.AIG_StartDate = employeeInfoDB.AIG_StartDate;
                        //employee.AIG_TerminationDate = employeeInfoDB.AIG_TerminationDate;
                        
                        //// CHECK ON DB todo check the value is valid or not
                        //employee.TerminationTypeId = employeeInfoDB.TerminationTypeId;

                        //employee.LOA_Date = employeeInfoDB.LOA_Date;
                        //employee.ReInstateDate = employeeInfoDB.ReInstateDate;
